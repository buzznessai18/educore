...
const UPDATE_STUDENT_FEE_PILLS: ActionPill[] = [
...
export function UpdateStudentFeePage() {
  ...
  const handleGetResult = () => {
    ...
    setSelectedFees([]);
    setSelectedStudents([]);
    toast.success("Fee and student masters loaded");
  };
  ...
  const handleSubmit = () => {
    ...
    if (selectedFees.length === 0 || selectedStudents.length === 0) {
      toast.error("Select at least one fee and one student");
      return;
    }
    toast.success(
      `Updated ${selectedFees.length} fee plan(s) for ${selectedStudents.length} student(s)`,
    );
  };
  ...
  return (
    <div className="mx-auto max-w-[1200px] space-y-4 pb-10">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold text-info">Update Student Fee</h2>
          <p className="text-sm text-muted-foreground">
            Home <span className="mx-1 text-muted-foreground/70">&gt;</span> Update Student Fee
          </p>
        </div>
        <ActionPills items={UPDATE_STUDENT_FEE_PILLS} />
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-card shadow-enterprise-sm">
        <div className="bg-info px-4 py-2.5">
          <h3 className="text-sm font-semibold text-white">Update Student Fee</h3>
        </div>
      ...
      <div className="space-y-4">
                  ...
                  >
                    <TableCell>
                      <Checkbox
                        ...
                        aria-label={`Add fee ${row.classLabel}`}
                      />
                    </TableCell>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
          ...
          <Button
            ...
            onClick={() => navigate({ to: "/fms/cancelled-fee-list" })}
          >
            Exit
          </Button>
        </div>
